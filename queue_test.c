...
// A estrutura "queueint_t" será usada com as funções de queue.c usando um
...
typedef struct queueint_t {
  struct queueint_t *prev; // ptr para usar cast com queue_t
  struct queueint_t *next; // ptr para usar cast com queue_t
  int id;
  // outros campos podem ser acrescidos aqui
} queueint_t;

queueint_t item[N];
queueint_t *fila0, *fila1, *aux, *final;
int ret;
...
int fila_correta(queueint_t *fila) {
  queueint_t *aux;
...
int main(int argc, char **argv, char **envp) {
  ...
  fila0 = NULL;
  for (i = 0; i < N * N; i++) {
    aux = (queueint_t *)malloc(sizeof(queueint_t));
    aux->id = i;
  ...
  // imprimir a fila
  printf("Saida esperada: [9<0>1 0<1>2 1<2>3 2<3>4 3<4>5 4<5>6 5<6>7 6<7>8 "
         "7<8>9 8<9>0]\n");
  queue_print("Saida gerada:   ", (queue_t *)fila0, print_elem);
